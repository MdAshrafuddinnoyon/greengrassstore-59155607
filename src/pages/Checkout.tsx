...
const Checkout = () => {
  ...
  const { shippingSettings, footerFeatures } = useSiteSettings();
  ...
  const activeFooterFeatures = (footerFeatures || []).filter((f) => f.enabled);
  const fallbackFeatures = [
    { id: 'default-delivery', icon: 'truck', title: 'Free Delivery', titleAr: 'توصيل مجاني', description: 'Free delivery above AED 200', descriptionAr: 'توصيل مجاني فوق 200 درهم', enabled: true },
    { id: 'default-secure', icon: 'credit-card', title: 'Secure Payment', titleAr: 'دفع آمن', description: '100% Secure Payment', descriptionAr: 'دفع آمن 100%', enabled: true },
  ];
...
Please confirm my order. Thank you!`;
  ...
  const handleHomeDeliveryOrder = async () => {
    ...
    if (!acceptedTerms) {
      toast.error(isArabic ? "يرجى الموافقة على الشروط والأحكام" : "Please accept the terms and conditions");
      return;
    }

    try {
      console.log('Creating order with info:', customerInfo);
      ...
      // Get current user for linking order (if logged in)
      const { data: { user } } = await supabase.auth.getUser();
      
      // Generate customer ID or use authenticated user ID
      const customerId = user?.id || `guest-${Date.now().toString(36)}`;
      
      const orderData = {
        order_number: orderNumber,
        customer_name: customerInfo.name.trim(),
        customer_email: customerInfo.email?.trim() || `guest-${Date.now()}@guest.local`,
        customer_phone: customerInfo.phone.trim(),
        customer_address: `${customerInfo.address.trim()}, ${customerInfo.city.trim()}`,
        items: orderItems,
        subtotal: subtotal,
        shipping: shipping,
        tax: 0,
      ...
      // Create order with guest customer info
      ...
      try {
        for (const item of items) {
          ...
          if (fetchError) {
            console.warn(`Failed to fetch stock for product ${productId}:`, fetchError);
            continue;
          }
          ...
          if (productData) {
            ...
            if (updateError) {
              console.warn(`Failed to update stock for product ${productId}:`, updateError);
            } else {
      ...
      // Auto-create/update customer profile if not logged in
      if (!user) {
        try {
          ...
          const { data: existingProfile } = await supabase
            .from('profiles')
            .select('id, user_id')
            .eq('email', customerInfo.email?.trim() || `guest-${Date.now()}@guest.local`)
            .maybeSingle();
          ...
          if (!existingProfile) {
            ...
            const { error: profileError } = await supabase
              ...
              .insert({
                ...
                email: customerInfo.email?.trim() || `guest-${Date.now()}@guest.local`,
            ...
            if (profileError) {
              console.warn('Failed to create customer profile:', profileError);
              // Don't fail the order creation if profile creation fails
            } else {
              console.log('Customer profile created successfully');
            }
          }
        } catch (profileError) {
          console.warn('Error during customer profile creation:', profileError);
          // Don't fail the order if profile creation has issues
        }
      }
      ...
      toast.success(
        isArabic 
          ? `تم تأكيد طلبك! رقم الطلب: ${orderNumber}` 
      ...
      toast.error(
        isArabic 
          ? `حدث خطأ في إنشاء الطلب: ${errorMessage}` 
          ...
          <div className="grid lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-2 space-y-6">
              ...
              >
                ...
                <div className="divide-y">
                  {items.map((item) => {
                    const imageUrl = item?.product?.images?.edges?.[0]?.node?.url || item?.image;
                    ...
                    <div key={item.variantId} className="py-3 sm:py-4 flex gap-3 sm:gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="font-semibold text-[#2d5a3d] mt-1 text-sm sm:text-base">
                          {currency} {parseFloat(item?.price?.amount || '0').toFixed(2)}
                        </p>
                      </div>
              ...
              >
                <div className="flex items-center justify-between mb-3 sm:mb-4">
                  ...
                  <span className="px-2.5 py-1 bg-green-100 text-green-700 text-xs font-medium rounded-full">
                    {isArabic ? "لا حاجة لحساب" : "No Account Needed"}
                  </span>
                </div>
                <p className="text-xs sm:text-sm text-gray-500 mb-3 sm:mb-4">
                  {isArabic 
                    ? "يمكنك الطلب بدون إنشاء حساب. سنرسل لك رابط تتبع الطلب."
                    : "Order without creating an account. We'll send you an order tracking link."
                  }
                </p>
            ...
            <div className="lg:col-span-1">
              ...
              >
                ...
                {/* Trust Badges (dynamic from footer settings) */}
                <div className="mt-4 sm:mt-6 pt-4 sm:pt-6 border-t space-y-3">
                  {(activeFooterFeatures.length ? activeFooterFeatures : fallbackFeatures)
                    .slice(0, 4)
                    .map((feature) => {
                      const Icon = featureIconMap[feature.icon as keyof typeof featureIconMap] || Shield;
                      const title = isArabic ? feature.titleAr : feature.title;
                      const description = isArabic ? feature.descriptionAr : feature.description;
                      return (
                        <div key={feature.id} className="flex items-center gap-3 text-xs sm:text-sm text-gray-600">
                          <div className="w-7 h-7 sm:w-9 sm:h-9 bg-muted rounded-lg flex items-center justify-center flex-shrink-0">
                            <Icon className="w-4 h-4 sm:w-5 sm:h-5 text-primary" />
                          </div>
                          <div className="space-y-0.5">
                            <p className="font-medium text-foreground text-sm sm:text-base">{title}</p>
                            <span className="text-xs sm:text-sm text-muted-foreground block leading-tight">{description}</span>
                          </div>
                        </div>
                      );
                    })}
