...
import { Loader2, MessageSquare, Bot, Store, Save, RefreshCw, Truck, RotateCcw, CreditCard, MapPin, Plus, Trash2, Percent, Sparkles } from "lucide-react";
...
export const SiteSettingsManager = () => {
  const { whatsapp, salesAgent, storeInfo, footerFeatures, paymentBanner, shippingSettings: contextShippingSettings, featuredCategorySection, loading: contextLoading, refetch } = useSiteSettings();
  const [featuredCategorySectionLocal, setFeaturedCategorySectionLocal] = useState(featuredCategorySection);
  ...
  const [paymentBannerLocal, setPaymentBannerLocal] = useState(paymentBanner);
  ...
  useEffect(() => { setWhatsappSettings(whatsapp); }, [whatsapp]);
  useEffect(() => { setSalesAgentSettings(salesAgent); }, [salesAgent]);
  useEffect(() => { setStoreInfoLocal(storeInfo); }, [storeInfo]);
  useEffect(() => { setFooterFeaturesLocal(footerFeatures); }, [footerFeatures]);
  useEffect(() => { setShippingSettings(contextShippingSettings); }, [contextShippingSettings]);
  useEffect(() => { setPaymentBannerLocal(paymentBanner); }, [paymentBanner]);
  useEffect(() => { setFeaturedCategorySectionLocal(featuredCategorySection); }, [featuredCategorySection]);
  ...
  // Fallback for missing context data
  if (!featuredCategorySection || !whatsapp || !storeInfo) {
    return (
      <div className="flex flex-col items-center justify-center py-12">
        <Loader2 className="w-8 h-8 animate-spin text-primary mb-4" />
        <p className="text-destructive">Site settings could not be loaded. Please check your connection or try again later.</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      <Tabs defaultValue="whatsapp" className="space-y-4">
        ...
        <TabsTrigger value="home-category" className="gap-2">
          <Sparkles className="w-4 h-4" />
          Home Category
        </TabsTrigger>
      </TabsList>

      {/* Home Category Display Settings moved to HomepageSectionsManager */}
      {/* WhatsApp Settings */}
