...
export const MediaLibrary = () => {
  ...
  const handleBulkDelete = async () => {
    ...
    if (!confirm(`Are you sure you want to delete ${selectedFileIds.length} selected file(s)?`)) return;
    try {
      // Get selected files
    ...
    <div className="space-y-6">
      ...
      {selectedFileIds.length > 0 && (
        <div className="flex items-center gap-2 mb-2">
          <span className="text-sm">{selectedFileIds.length} selected</span>
          ...
          <select
            ...
            onChange={async (e) => {
              ...
              try {
                // Update folder for selected files
                const { error } = await supabase
                  .from('media_files')
                  .update({ folder: targetFolder })
                  .in('id', selectedFileIds);
                if (error) throw error;
                toast.success(`Moved ${selectedFileIds.length} file(s) to '${targetFolder}'`);
                setSelectedFileIds([]);
                fetchFiles();
              } catch (err) {
