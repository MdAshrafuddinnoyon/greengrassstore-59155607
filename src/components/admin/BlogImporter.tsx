...
import { Upload, FileText, Download, AlertCircle, CheckCircle2, Loader2, FileSpreadsheet, ShoppingBag, Store } from "lucide-react";
import { Alert, AlertDescription, AlertTitle } from "@/components/ui/alert";
import { Tabs, TabsContent, TabsList, TabsTrigger } from "@/components/ui/tabs";
import { Badge } from "@/components/ui/badge";
import { Progress } from "@/components/ui/progress";

type ImportSource = 'wordpress-xml' | 'wordpress-csv' | 'shopify-blog' | 'standard-csv';
...
export const BlogImporter = ({ onImportComplete }: BlogImporterProps) => {
  ...
  // Parse Shopify Blog CSV format
  const parseShopifyBlogCSV = (text: string): BlogPost[] => {
    const lines = text.split('\n').filter(line => line.trim());
    if (lines.length < 2) return [];
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      blogs.push({
        title,
        content: data['body_html'] || data['body_(html)'] || data['content'] || '',
        excerpt: data['summary_html'] || data['excerpt'] || '',
        slug: data['handle'] || generateSlug(title),
        date: data['published_at'] || data['created_at'] || new Date().toISOString(),
        author: data['author'] || 'Green Grass Team',
        category: data['blog'] || data['blog_title'] || 'General',
        tags: (data['tags'] || '').split(',').map(t => t.trim()).filter(Boolean),
        status: data['published'] === 'true' || data['published'] === 'TRUE' ? 'published' : 'draft',
        featuredImage: data['image_src'] || data['featured_image'] || ''
      });
    }
  ...
  const detectFileType = (content: string, fileName: string): ImportSource => {
    ...
    if (firstLine.includes('handle') && (firstLine.includes('body_html') || firstLine.includes('published_at'))) {
      return 'shopify-blog';
    }
    ...
    reader.onload = (event) => {
      ...
      switch (source) {
        ...
        case 'wordpress-csv':
          ...
          break;
        case 'shopify-blog':
          parsedBlogs = parseShopifyBlogCSV(text);
          break;
        default:
...
"Sample Blog Post","<p>Your blog content with HTML.</p>","Brief excerpt","sample-blog-post","2024-01-15 10:00:00","Author Name","publish","Plant Care","tips,indoor plants","https://example.com/image.jpg"`;
      ...
      case 'shopify-blog':
        template = `Handle,Title,Body (HTML),Summary HTML,Author,Blog,Tags,Published,Published At,Image Src
"sample-blog-post","Sample Blog Post","<p>Your blog content with HTML.</p>","Brief excerpt","Author Name","News","tips,indoor plants","true","2024-01-15T10:00:00Z","https://example.com/image.jpg"`;
        filename = 'shopify_blog_template.csv';
        break;
...
"Sample Blog Post","مقال نموذجي","<p>Your blog content with HTML.</p>","<p>محتوى المدونة</p>","Brief excerpt","مقتطف مختصر","sample-blog-post","Author Name","Plant Care","tips,indoor plants","published","https://example.com/image.jpg","2024-01-15"`;
  ...
  const getSourceBadge = () => {
    switch (detectedSource) {
      ...
      case 'shopify-blog':
        return <Badge className="bg-green-100 text-green-800"><ShoppingBag className="w-3 h-3 mr-1" /> Shopify Blog</Badge>;
      default:
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Import blog posts from WordPress, Shopify, or CSV
          </p>
        </div>
      </div>
      ...
      <Tabs defaultValue="wordpress-xml" className="w-full">
        <TabsList className="grid w-full grid-cols-4">
          ...
          <TabsTrigger value="shopify-blog" className="text-xs sm:text-sm">Shopify</TabsTrigger>
        ...
        <TabsContent value="shopify-blog" className="space-y-4">
          <Card>
            <CardHeader className="pb-3">
              <CardTitle className="text-sm font-medium flex items-center gap-2">
              ...
              <CardDescription>
                Export from Shopify Admin: Online Store → Blog Posts → Export
              </CardDescription>
            </CardHeader>
            <CardContent>
              <Button variant="outline" size="sm" onClick={() => downloadTemplate('shopify-blog')}>
                <Download className="w-4 h-4 mr-2" />
      ...
      <Alert>
        ...
        <AlertDescription className="space-y-2">
          <ul className="list-disc list-inside text-sm space-y-1">
            ...
            <li><strong>Shopify Blog:</strong> Online Store → Blog Posts → Export</li>
