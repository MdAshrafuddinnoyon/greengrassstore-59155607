...
import { Upload, FileSpreadsheet, Download, AlertCircle, CheckCircle2, Loader2, ExternalLink } from "lucide-react";
...
export const ProductImporter = () => {
  ...
  const handleImport = async () => {
    ...
    // Note: For actual Shopify import, you would need to call Shopify Admin API
    // This is a simulation showing what would be imported
    toast.info('CSV Import Preview', {
      description: `To import ${csvData.length} products to Shopify, use the Shopify Admin panel or ask the AI assistant to create products from this data.`
    });
    ...
    for (const product of csvData) {
      if (product.title) {
        ...
        result.failed++;
        result.errors.push(`Missing title for product`);
      }
    }
...
"Ceramic Pot","<p>Elegant ceramic pot</p>","Green Grass","Pots","ceramic, home",19.99,POT-001,1.2,Color,White,https://example.com/pot.jpg`;
    ...
    a.href = url;
    a.download = 'shopify_product_template.csv';
    a.click();
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Import products from CSV file (Shopify format)</p>
        </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Upload a CSV file with product data in Shopify format
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div 
      ...
      {importResult && (
        <Alert variant={importResult.failed > 0 ? "destructive" : "default"}>
          ...
          <AlertDescription>
            <p>{importResult.success} products processed successfully, {importResult.failed} failed.</p>
      ...
      <Alert>
        ...
        <AlertDescription className="space-y-2">
          <p>Products are synced with your Shopify store. You can:</p>
          <ul className="list-disc list-inside text-sm space-y-1">
            <li>Upload CSV here to preview and validate</li>
            <li>Import directly via Shopify Admin</li>
            <li>Ask AI assistant to create products from your CSV data</li>
          </ul>
