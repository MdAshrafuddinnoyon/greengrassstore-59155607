...
export function LocalProductCard({ product, isArabic = false }: LocalProductCardProps) {
  ...
  const handleToggleWishlist = (e: React.MouseEvent) => {
    ...
    if (isWishlisted) {
      ...
      {/* Image Container - Fixed height, wrapped in Link */}
      ...
      {/* Content - Fixed height for consistent cards, NOT inside Link */}
      <div className="p-3 sm:p-4 flex flex-col h-[140px] sm:h-[160px]">
        <button
          type="button"
          onClick={(e) => {
            e.stopPropagation();
            navigate(`/shop?category=${product.category.toLowerCase()}`);
          }}
          className="text-[10px] sm:text-xs text-muted-foreground uppercase tracking-wide mb-0.5 sm:mb-1 hover:text-primary transition-colors text-left bg-transparent border-0 p-0 cursor-pointer"
          tabIndex={0}
        >
          {product.category}
        </button>
