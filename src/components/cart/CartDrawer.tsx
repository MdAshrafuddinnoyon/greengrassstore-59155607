            ...
            <>
              ...
              <div className="flex-1 overflow-y-auto pr-2 min-h-0">
                <div className="space-y-4">
                  {items.map((item) => {
                    // Fallbacks for image, title, options, price, currency
                    const imageUrl = item?.product?.node?.images?.edges?.[0]?.node?.url || item?.image || '/placeholder.svg';
                    const productTitle = item?.product?.node?.title || item?.title || 'Product';
                    const selectedOptions = (item?.selectedOptions || []).map(option => option.value).join(' • ');
                    const currency = item?.price?.currencyCode || 'AED';
                    const price = parseFloat(item?.price?.amount || '0');
                    return (
                      <div key={item.variantId} className="flex gap-3 sm:gap-4 p-2 border-b items-center">
                        <div className="w-16 h-16 sm:w-20 sm:h-20 bg-secondary/20 rounded-md overflow-hidden flex-shrink-0">
                          <img
                            src={imageUrl}
                            alt={productTitle}
                        ...
                        <div className="flex-1 min-w-0">
                          <h4 className="font-medium truncate text-base sm:text-lg">{productTitle}</h4>
                          <div className="text-xs sm:text-sm text-muted-foreground overflow-x-auto whitespace-nowrap">
                            {selectedOptions}
                          </div>
