...
export const WhatsAppButton = ({ message }: WhatsAppButtonProps) => {
  ...
  const defaultMessage = "Hello! I'm interested in your products. Can you help me?";
  
  const handleSendMessage = () => {
    const messageToSend = customMessage.trim() || message || defaultMessage;
                ...
                <div className="flex flex-wrap gap-1.5">
                  {quickMessages.map((msg, index) => (
                    <button
                      ...
                      className="text-xs bg-white px-2.5 py-1.5 rounded-full text-gray-600 hover:bg-gray-100 transition-colors border border-gray-200"
                    >
                      {msg}
                    </button>
                  ))}
            ...
            <div className="p-3 bg-white border-t border-gray-100">
              <div className="flex items-center gap-2">
                <input
                  ...
                  className="flex-1 px-3 py-2 bg-gray-100 rounded-full text-sm focus:outline-none focus:ring-2 focus:ring-[#25D366]/50"
                />
                <button
        ...
        <AnimatePresence mode="wait">
          {!isOpen && (
            <motion.span
              ...
              exit={{ opacity: 0, x: 10 }}
              className="mr-3 bg-white px-4 py-2 rounded-full shadow-lg text-sm font-medium text-gray-700 hidden sm:block"
            >
