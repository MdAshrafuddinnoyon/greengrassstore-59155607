...
export const MegaMenuManager = () => {
  ...
  const fetchMediaFiles = async () => {
    try {
      ...
      setMediaFiles((data || []).map(f => ({ id: f.id, file_path: f.file_url, file_name: f.file_name })));
    } catch (error) {
  ...
  const fetchContent = async () => {
    ...
    try {
      ...
      if (data) {
        setCategories(data.value as unknown as MegaMenuCategory[]);
      }
    } catch (error) {
    ...
    try {
      ...
      if (existing) {
        const { error } = await supabase
          .from('site_settings')
          .update({ value: JSON.parse(JSON.stringify(categories)) })
          .eq('key', 'mega_menu_categories');
        if (error) throw error;
      } else {
        const { error } = await supabase
          .from('site_settings')
          .insert({ key: 'mega_menu_categories', value: JSON.parse(JSON.stringify(categories)) });
        if (error) throw error;
      }
