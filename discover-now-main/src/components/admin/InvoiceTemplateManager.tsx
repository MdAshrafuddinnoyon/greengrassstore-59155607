...
export const InvoiceTemplateManager = () => {
  ...
  const fetchSettings = async () => {
    try {
      ...
      if (data?.value) {
        setSettings({ ...defaultInvoiceTemplate, ...data.value as unknown as InvoiceTemplateSettings });
      }
    } catch (error) {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on(
        'postgres_changes',
        { event: '*', schema: 'public', table: 'site_settings', filter: "key=eq.invoice_template" },
        () => fetchSettings()
      )
      .subscribe();
  ...
  const saveSettings = async () => {
    ...
    try {
      ...
      if (existing) {
        ...
        const { error } = await supabase
          .from('site_settings')
          .insert({ key: 'invoice_template', value: jsonValue });
        if (error) throw error;
      }
