  ...
  const saveSettings = async (key: string, value: object) => {
    ...
    try {
      ...
      if (existing) {
        ...
        const { error } = await supabase
          .from('site_settings')
          .insert({ key: key, value: JSON.parse(JSON.stringify(value)) });
        if (error) throw error;
      }
