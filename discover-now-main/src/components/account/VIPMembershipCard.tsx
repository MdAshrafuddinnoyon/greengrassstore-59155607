...
export const VIPMembershipCard = ({ userId }: VIPMembershipCardProps) => {
  ...
  const memberSpend = member.total_spend || 0;
  const tierMinSpend = tier.min_spend || 0;
  const nextTierMinSpend = nextTier?.min_spend || 0;
  const progressToNext = nextTier 
    ? Math.min(((memberSpend - tierMinSpend) / (nextTierMinSpend - tierMinSpend)) * 100, 100)
    : 100;
  const amountToNextTier = nextTier ? Math.max(nextTierMinSpend - memberSpend, 0) : 0;

  // Calculate expiry date (1 year from member_since)
  const tierUpdateDate = member.member_since ? new Date(member.member_since) : new Date(member.created_at || new Date());
    ...
    >
        ...
        <div className="relative z-10 h-full p-5 sm:p-6 flex flex-col text-white">
          ...
          <div className="flex items-end justify-between">
            ...
            <div className="text-right">
              ...
              <p className="text-xl sm:text-2xl font-bold">{tier.discount_percent || 0}%</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-3 mt-4">
        <div className="bg-card rounded-xl p-3 border border-border/50 text-center">
          ...
          <p className="font-semibold text-foreground">AED {(member.total_spend || 0).toFixed(0)}</p>
        </div>
        <div className="bg-card rounded-xl p-3 border border-border/50 text-center">
          ...
          <p className="font-semibold text-foreground">{(member.points_earned || 0) - (member.points_redeemed || 0)}</p>
        </div>
        <div className="bg-card rounded-xl p-3 border border-border/50 text-center">
